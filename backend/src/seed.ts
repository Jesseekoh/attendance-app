...
async function generateDepartments() {
  try {
    const formattedDepartments = departments.map((dep) => ({ name: dep }));
    await prisma.department.createMany({ data: formattedDepartments });
  } catch (error) {
...
async function generateCourses() {
  ...
  try {
    await prisma.course.createMany({ skipDuplicates: true, data: courses });
  } catch (error) {
